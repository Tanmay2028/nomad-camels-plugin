...
def test_parse_CAMELS_file_exact_values():
    ...
    data = parser.parse('tests/data/raw/test_CAMELS_file.nxs', archive, logging.getLogger(), testing=True)

    # Assert basic attributes
    assert data.name == "test_CAMELS_file"
    assert data.datetime.isoformat() == "2025-03-04T17:14:44.175182+00:00"
    assert data.measurement_description == "Measurement&nbsp;Description&nbsp;Test&nbsp;Entry<br>New&nbsp;Line"
    assert data.protocol_description == "This&nbsp;is&nbsp;the&nbsp;protocol&nbsp;description.<br>New&nbsp;Line."
    assert data.protocol_overview == ("Sweep&nbsp;'Simple_Sweep'&nbsp;demo_motorX:<br>"
    ...
    settings = json.loads(data.camels_instrument_settings)
    ...
    if hasattr(data.samples[0], 'reference') and not data.samples[0].reference is None:
    ...
    for instrument in data.instruments:
        ...
        if hasattr(instrument, 'reference') and not instrument.reference is None:
