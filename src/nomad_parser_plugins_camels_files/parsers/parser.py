        ...
        *_, self._fname = mainfile.rsplit("/", 1)
        ...
        with h5py.File(mainfile, "r") as hdf5_file:
            ...
            try:
                ...
                self._logger.warning("No NOMAD sample found in the CAMELS file")
                try:
                    sample_id_bytes = hdf5_file[self.camels_entry_name]["sample"][
                        "sample_id"
                    ][()]
                    ...
                    self._logger.warning("No regular sample found in the CAMELS file")
            ...
            try:
                ...
                self._logger.warning("No NOMAD user found in the CAMELS file")
                user_id_bytes = hdf5_file[self.camels_entry_name]["user"]["name"][()]
                if isinstance(user_id_bytes, int) or isinstance(user_id_bytes, np.int64):
                    user_id_string = user_id_bytes.decode("utf-8")
                else:
                    user_id_string = user_id_bytes.decode("utf-8")
